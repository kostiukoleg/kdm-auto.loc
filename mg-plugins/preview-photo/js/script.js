...
var previewPhoto = (function() {
  ...
  return { 
     ...
     */
    start:function(nextItem, total_count, imgCount, data = '') {
      $('.generation').show();
      $('.progress').show();      
      ...
      function(response) {
        ...
        if(response.data.percent<100){
          previewPhoto.start(response.data.nextItem, response.data.total_count, response.data.imgCount, response.data.data);        
        }else{
          $('.generation').hide();
          $('.progress').text('Выполнено 0%');
          $('.generation').hide();
