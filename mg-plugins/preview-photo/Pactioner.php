...
class Pactioner extends Actioner {
  ...
  public function processNew() {
    ...
    if(!$_POST['data']['allProductCount']) {
      $res = DB::query('SELECT COUNT(id) FROM '.PREFIX.'product');
      if($row = DB::fetchAssoc($res)) {
        $_POST['data']['allProductCount'] = $row['COUNT(id)'];
      }
    }
    ...
    while($_POST['data']['workCount'] <= $_POST['data']['allProductCount']) {
      $res = DB::query('SELECT p.id, p.image_url,
        (SELECT GROUP_CONCAT(pv.image SEPARATOR \'|\') FROM '.PREFIX.'product_variant AS pv WHERE pv.product_id = p.id) AS images
        FROM '.PREFIX.'product AS p 
        WHERE p.id > '.DB::quoteInt($_POST['data']['lastId']).' ORDER BY p.id ASC LIMIT 1');
      if($row = DB::fetchAssoc($res)) {
        $_POST['data']['lastId'] = $row['id'];

        $images = explode('|', $row['image_url']);
        $imagesV = explode('|', $row['images']);

        foreach ($imagesV as $key => $value) {
          $images[] = $value;
        }
      ...
      $dir = SITE_DIR.'uploads/product/'.floor($_POST['data']['lastId']/100).'00'.$ds.$_POST['data']['lastId'].'/';

      foreach ($images as $key => $image) {
        if(!$image) continue;

        if(!file_exists($dir.$image)) {
          $log .= "\n Файл ".$dir.$image.' отсутсвует.';
          continue;
        }
        if(!$this->validateImg($dir.$image)) {
          $log .= "\n Файл ".$dir.$image.' не является корректным изображением.';
          continue;
        }
        ...
        if ($options["watter"] == 'true') {
          $this->addWatterMark($dir.$image);
        }
        // создаем две миниатюры
        $this->reSizeImage('70_'.$image, $dir.$image, $options['width70'], $options['height70'], $dir.'thumbs/');
        $this->reSizeImage('30_'.$image, $dir.$image, $options['width30'], $options['height30'], $dir.'thumbs/');
      }

      $_POST['data']['workCount']++;
      $counter++;

      $execTime = microtime(true) - $this->startTime;
      if($execTime + 10 >= $this->maxExecTime) break;
    }

    $percent = floor(($_POST['data']['workCount'] * 100) / $_POST['data']['allProductCount']);
    $data = array(
      'percent' => $percent,
      'total_count' => $_POST['data']['allProductCount'],
      'nextItem' => $_POST['data']['workCount'],
      'imgCount' => $counter,
      'log' => $log,
      'data' => $_POST['data']
    );
      ...
      while($product = DB::fetchAssoc($dbRes)){
        ...
        foreach($images as $key => $image){		
          ...
          if(!empty($image) && file_exists($path . DIRECTORY_SEPARATOR . $image)){
            if (!self::validateImg($path . DIRECTORY_SEPARATOR . $image)) {
              unset($images[$key]);
              $log .= "\n $imgCount Файл ".$image.' не является корректным изображением.';
              $imgCount++;
              continue;
            }
  ...
  private function reSizeImage($name, $tmp, $widthSet, $heightSet, $dirUpload = 'uploads/thumbs/') {
    ...
    switch ($ext) {
      ...
      default:
        ...
        imagejpeg($image_p, $dirUpload . $name . '.' . $ext, $options['quality']);
