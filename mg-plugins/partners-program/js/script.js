...
var partner = (function () {
  return {
    ...
    init: function () {
      ...
      // Выплата.
      $('body').on('click', '#request-modal .save-button.request', function () {
        partner.paymentToPartner($(this).attr('id'));
      });

      /** изменение информации о партнере */
      $('body').on('click', '#partner-modal .save-button.partner', function () {
      ...
      $('.admin-center').on('click', '.section-partners .list-request', function () {
        ...
        $('.list-partners').removeClass('success');
        $('#list-request-table').slideToggle();
        $('.list-request').toggleClass('success');
        admin.refreshPanel();
      });
      ...
      $('body').on('click', '.b-modal_close.request', function () {
        $('.admin-center #partner-modal').css('z-index', 99);
        admin.closeModal($(this).closest('#request-modal'));
      ...
      $('body').on('click', '.b-modal-close.partner', function () {
        admin.closeModal($(this).closest('#partner-modal'));
      });
    ...
    paymentToPartner: function (id) {
      ...
      if (confirm(message)) {
        ...
        function (response) {
          ...
          admin.closeModal($('.product-table-wrapper').closest('#request-modal'));
          $('.admin-center #partner-modal').css('z-index', 99);
        }
        );
      }
    ...
    fillFiledsRequestModal: function () {
      ...
      return (function (response) {
        ...
        admin.openModal($('#request-modal'));
        $('.admin-center #partner-modal').css('z-index', 10);
      })
