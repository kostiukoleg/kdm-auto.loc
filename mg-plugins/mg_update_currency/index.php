...
//    mgAddAction('mg_gethtmlcontent', array(__CLASS__, 'autoUpdateProductPrice'), 1, 1);
  ...
  static function updateCurrency() { 
    ...
    $xmlData = updateCurrency::getFileByUrl('http://www.cbr.ru/scripts/XML_daily.asp?date_req='.date('d-m-Y'));
  ...
  static function autoUpdateHandle() {
    ...
    $updateExpTime = MG::getSetting('CBUpdateTime'); //вермя обновления задается в часах, приводим к секундам
    
    if($updateExpTime > time()) {
      return false;
    } else {
      $_SESSION['update_currency_process'] = 1;
      
      if(updateCurrency::updateCurrency()) {
        MG::setOption('CBUpdateTime', time() + 60 * 60 * 3);
      ...
      file_put_contents(dirname(__FILE__).DIRECTORY_SEPARATOR."last_update.txt", date('d.m.Y H:i:s'));
      Storage::clear();
