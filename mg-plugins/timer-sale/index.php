...
static function showTimerSale($args) {  
  if ($args['id']) {
    ...
    if (class_exists('Property')) {
      $sql = "SELECT p.price,p.old_price, prop.name FROM `".PREFIX."product` p "
        . "LEFT JOIN `".PREFIX."category` c ON c.id = p.cat_id "
        . "LEFT JOIN `".PREFIX."product_user_property_data` prop ON prop.`product_id`= p.id "
        . "WHERE p.id = ".DB::quote($args['id'])." AND prop.prop_id = ".DB::quote($options['propertyId']);   
      $res = DB::query($sql);
      if ($product = DB::fetchArray($res) ) {
        $time = $product['name'];
        $timest = strtotime($time);     
        if ($timest >= time()) {
          $percent = 100 - 100 * $product['price'] / $product['old_price'];
      ...
      $sql = "SELECT p.price,p.old_price, prop.value FROM `".PREFIX."product` p "
        . "LEFT JOIN `".PREFIX."category` c ON c.id = p.cat_id "
