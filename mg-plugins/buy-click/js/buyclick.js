  ...
  return {
    init: function () {
      ...
      $('body').on('click', '.mg-buy-click-button', function () {
        ...
        if ($(this).parents('.product-wrapper').length) {
          price = $(this).parents('.product-wrapper').find(".product-price .product-default-price:first").text()+"грн. ";
          if (price == '') {
            price = $(this).parents('.product-wrapper').find(".product-price").text();
          }
        }
        else {
          price = $('body').find(".product-status-list li .price:first").text()+"грн. ";
        ...
        $.ajax({
          ...
          success: function (response) {
            if ('success' == response.status) {
              $('.mg-action-buttons').before("<span data-buy="+id+" class='loading-send-order'>Спасибо за покупку! Если у Вас возникнут какие-либо вопросы, <br>свяжитесь с нами по тел. +38 (096) 938-96-28 или +38 (063) 313-49-82</span>");
              $('.mg-action-buttons .mg-send-order-click-button').hide();
