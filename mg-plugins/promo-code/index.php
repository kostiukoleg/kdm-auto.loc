...
/*
  ...
  Description: Плагин позволяет сделать скидку с помощью ввода специального промокода. Разместите следующий шорткод  [promo-code] в том месте, где должно располагаться поля для ввода промокода на странице оформления заказа или корзины. В большинстве шаблонов шорткод уже добавлен.
  Author: Avdeev Mark
  Version: 2.2.7
 */
...
class PromoCode {
  ...
  public function __construct() {
  	...
  	if (!empty($_POST['couponCode']) && (strpos($_SERVER['REQUEST_URI'], '/order') !== false || strpos($_SERVER['REQUEST_URI'], '/cart') !== false)) {
